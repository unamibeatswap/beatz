...
class ISRCManager {
    ...
    async initialize() {
        ...
        try {
            // Try to establish a backend session if possible (non-blocking)
            try {
                const BackendClient = require('./backend-client')
                const backend = new BackendClient()
                try { backend.loadSession() } catch(e){}
                if (!backend.sessionToken) {
                    try {
                        let idToken = null
                        if (window.unifiedAuth && typeof window.unifiedAuth.getIdToken === 'function') {
                            idToken = await window.unifiedAuth.getIdToken()
                        } else if (window.authManager && typeof window.authManager.getIdToken === 'function') {
                            idToken = await window.authManager.getIdToken()
                        } else if (typeof EnhancedAuthenticationManager !== 'undefined') {
                            const am = new EnhancedAuthenticationManager()
                            if (am && typeof am.isAuthenticated === 'function' && am.isAuthenticated()) {
                                idToken = await am.getIdToken()
                            }
                        }
                        if (idToken) {
                            await backend.createSession(idToken)
                            console.log('✅ MCP session established during ISRCManager.init')
                        }
                    } catch (se) {
                        console.warn('Failed to create backend session during init:', se)
                    }
                }
            } catch (be) {
                // ignore backend client errors
            }
    ...
    async generateISRC(trackTitle = '', artistName = '') {
        try {
            ...
            // Prefer server-generated ISRC when MCP backend is available
            try {
                const BackendClient = require('./backend-client')
                const backend = new BackendClient()

                // Attempt to load existing session token from localStorage
                try { backend.loadSession() } catch (lerr) { /* ignore */ }

                // If no session token, try to exchange a client idToken with MCP
                if (!backend.sessionToken) {
                    let idToken = null
                    try {
                        if (window.unifiedAuth && typeof window.unifiedAuth.getIdToken === 'function') {
                        ...
                        console.warn('Failed to obtain client idToken for session exchange:', idErr)
                    }

                    if (idToken) {
                        try {
                            await backend.createSession(idToken)
                            console.log('✅ Backend session created via token-exchange')
                        } catch (csErr) {
                            console.warn('Token-exchange createSession failed:', csErr)
                        }
                    }
                }

                const payload = { title: trackTitle, artist: artistName, userId: (window.currentUser && window.currentUser.id) || 'anonymous' }
                const resp = await backend.postJSON('/api/isrc/generate', payload)
                if (resp && resp.success && resp.isrc) {
                    console.log('✅ Received ISRC from MCP backend:', resp.isrc)
                    // store in local registry for offline resilience
                    this.registry.codes[resp.isrc] = { trackTitle: this.sanitizeInput(trackTitle), artistName: this.sanitizeInput(artistName), generated: new Date().toISOString(), used: false, validated: true }
                    await this.saveRegistry()
                    return resp.isrc
                }
            } catch (e) {
                // backend not reachable or failed - continue with local generation
                console.warn('MCP ISRC generation failed, falling back to local generation:', e && e.message)
            }
