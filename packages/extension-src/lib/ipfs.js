...
class IPFSManager {
    constructor() {
        // Do not embed Pinata keys in client code. Use a server-side upload/pinning endpoint.
    ...
    async uploadFile(file, metadata = {}) {
        try {
            ...
            // If server-side endpoint is available, delegate upload to MCP backend
            if (!this.pinataApiKey || !this.pinataSecretKey) {
                try{
                    // Lazy-require to avoid bundling issues
                    const BackendClient = require('./backend-client')
                    const backend = new BackendClient()
                    const meta = { title: pinataMetadata.name, keyvalues: pinataMetadata.keyvalues }
                    const result = await backend.uploadFile('/api/upload', file, meta)
                    if(result && result.success){
                        return { success: true, ipfsHash: result.file?.ipfsHash || (result.file && result.file.IpfsHash) || null, ipfsUrl: result.file && result.file.ipfsUrl || null, backend: true }
                    }
                }catch(err){
                    console.warn('Backend upload failed, falling back to client attempt:', err && err.message)
                }
                // If backend not available or failed, throw to indicate client-side disabled
                throw new Error('Client-side IPFS uploads are disabled. Please upload via a secure server-side endpoint that pins to IPFS.');
            }
    ...
    async getFileInfo(ipfsHash) {
        try {
            if (!this.pinataApiKey || !this.pinataSecretKey) {
                throw new Error('Pinata keys not configured in client. Use server-side API to query pin status.');
            }
    ...
    async unpinFile(ipfsHash) {
        try {
            if (!this.pinataApiKey || !this.pinataSecretKey) {
                throw new Error('Pinata keys not configured in client. Use server-side API to unpin.');
            }
    ...
    async testConnection() {
        try {
            if (!this.pinataApiKey || !this.pinataSecretKey) {
                console.warn('Pinata keys not present in client - skipping pinata authentication test');
                return false;
            }
