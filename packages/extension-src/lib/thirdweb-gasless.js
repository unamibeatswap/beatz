...
class ThirdwebGaslessManager {
    ...
    async canGaslessMint(userAddress) {
        ...
        try {
            // Prefer server-side relayer status if MCP backend available
            try {
                const BackendClient = require('./backend-client')
                const backend = new BackendClient()
                const resp = await backend.postJSON('/api/thirdweb/status', { address: userAddress })
                if (resp && resp.success) return { allowed: resp.allowed, reason: resp.reason || null, relayerBalance: resp.balance || 0 }
            } catch (e) {
                // ignore and fall back to SDK relayer check
            }
    ...
    async gaslessMint(userWallet, metadataUri, metadata = {}) {
        try {
            ...
            // Try server-side MCP gasless mint endpoint first
            try {
                const BackendClient = require('./backend-client')
                const backend = new BackendClient()
                const resp = await backend.postJSON('/api/thirdweb/mint', { to: userWallet, metadataUri, metadata })
                if (resp && resp.success) {
                    return resp.result || { transactionHash: resp.txHash || null, tokenId: resp.tokenId || null, gasless: true }
                }
            } catch (e) {
                console.warn('Backend gasless mint failed or not available, falling back to client SDK:', e && e.message)
            }
